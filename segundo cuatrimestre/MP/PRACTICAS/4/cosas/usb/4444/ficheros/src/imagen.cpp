	...
	if (salida) // con este if comprobamos que en el   tamanio sea mas pequeño que maxlong, asi nos aseguramos que cabe y no se nos repite la imagen dos veces
	{
		for(int i=0 ; i < nfilas ; i++)
		{
			for(int j=0;j<ncolumnas  ; j++)
			{
				...
				lectura++ ;	//cada vez que escribimos en arteASCII[] incrementamos lectura para la nueva posicion a 	
						//escribir
				escritura++;	//incrementamos para ir avanzando una posicion de la columna de la nº fila del nuevo vector
						//(matriz) como queramos llamarlo
			}
			arteASCII[escritura] = '\n' ;//al final de cada final nos dice el pdf que hay que meter un \n(logico para que se 
						     //represente bien todo)
			escritura++ ; //incrementamos escritura pk se ha añadido un "\n"
