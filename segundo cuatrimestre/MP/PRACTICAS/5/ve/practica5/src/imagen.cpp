...
bool Imagen::listaAArteASCII(const Lista celdas){
	...
	int longitud = celdas.longitud() ;
  	...
  	//ofstream fsalida ;
		...
		char grises[celdas.getCelda(i).size() + 1] = "" ; 

		strcpy(grises, celdas.getCelda(i).c_str()) ; // con c_str pasamos strings a cadenas de C

		salida = aArteASCII(grises,arteASCII,lenght); //aArteASCII devuelve si se ha hecho bien la escritura.
