...
//-------------------------------------------------------------------------------------------------------------------
 Imagen::Imagen(int filas, int columnas) : datos(0)//importante poner datos==NULL cosa que crear no hace
{
...
Imagen Imagen::operator+(const Imagen & orig) const
{
	...
	/*suma.datos=new byte[suma.nfilas*suma.ncolumnas];
	int concatena=0;
	for(int i=0;i<this->ncolumnas*this->nfilas;i++)
	{
		suma.datos[concatena]=this->datos[i];			(((ESTA FORMA ME GENERA 4)))
		concatena++;
	}
	for(int i=0;i<orig.ncolumnas*orig.nfilas;i++)
	{
		suma.datos[concatena]=orig.datos[i];
		concatena++;
	}*/
	return suma;
}
...
{
	if(datos != 0)
	delete [] datos ;//podemos poner //destruir();
