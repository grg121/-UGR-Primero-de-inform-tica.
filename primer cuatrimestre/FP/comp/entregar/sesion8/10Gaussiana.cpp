...
double GaussianaDe(double x, double esperanza, double desviacion){
...
double AreaHasta(double x, double esperanza, double desviacion){
    double t = 1/1+bo*x ;
    double producto = b1*t+b2*pow(t,2)+b3*pow(t,3)+b4*pow(t,4)+b5*pow(t,5) ;
    double gaussiana = GaussianaDe(x, esperanza, desviacion) ;
...
int main (){
    ...
    double y = GaussianaDe(x,esperanza,desviacion) ;
    ...
    double area = AreaHasta(x, esperanza, desviacion);
